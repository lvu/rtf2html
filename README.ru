...
3.  Компиляция
    ...
    - Под UNIX:
      ...
      работает значительно (примерно в 3 раза) быстрее, если собирать ее
    ...
    - Под MS Windows:
      Можно использовать Makefile.bcc, чтобы скомпилировать программу
      компилятором Borland C++ 5.5 (наверное, и другими версиями, в т.ч.
      Borland C++ Builder). Я не рекомендую использовать RW STL, которая
      поставляется с Borland C++ 5.5 - она НАМНОГО медленнее, чем STLport.
      Так что лучше установите STLport и компилируйте с ним.
